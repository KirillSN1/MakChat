...
export default class ChatController{
    static async createOrEditMessage(chat:Chat,user:AppUser,message:{ id?:number, text:string, dateTime:number }){
        ...
        else chatMessage = await ChatMessage.create(chat.id,user.id,message.text,message.dateTime);
        return chatMessage;
    }
